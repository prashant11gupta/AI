...
public class RouteNewFunc {
	private static int totalDistance = 0;

	public String findRoute(String srcCity,String destCity, Storage st) {
		...
		while (!nodeQueue.isEmpty()) {
			...
			Node nextNode = getNextNode(src, st);
			if (nextNode != null) {
	...
	private Node getNextNode(Node src, Storage st) {
		...
		for (Edge e : src.adjancies) {
			...
			if (!dest.checked) {
				...
				/*
				 * (f = G+H) where G = ActualDist H = logBase2(Displacement Distance);
				 * 
				 */
				double hueristicDist = actualDist + logBase2(directDist);
		...
		for (Node key : bestNode.keySet()) {
			if (valueList.get(0).equals(bestNode.get(key))) {
				nextNode = key;
				totalDistance += nextNode.minDistance;
				break;
			}
		}
		return nextNode;
	...
	public double logBase2(int directDist){
		
		return (Math.log((double)directDist))/Math.log(2);
	}
